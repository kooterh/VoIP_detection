...
predict_path=('/VoIP_detection/VoIP_detection_code/python/classification/case6/predict')
...
test_x_i2=load_data2['mismatch2_x']
...
test_total_batch2=int(n_test2/batch_size1)
j_t=[i_t for i_t in range(test_total_batch2)]
test_data2=np.zeros((test_total_batch2,768*3))
test_label2=np.zeros((test_total_batch2,2))
for i_t in j_t:
    test_xs2=test_x2[i_t*batch_size1:(i_t+1)*batch_size1]
    test_ys2 =test_y2[i_t*batch_size1:(i_t+1)*batch_size1]
    test_xs2 = test_xs2.reshape((batch_size1,fbin,nframes))
    test_xs_i2=test_x_i2[i_t*batch_size1:(i_t+1)*batch_size1]
    test_xs_i2=test_xs_i2.reshape((batch_size1,fbin,nframes))
    test_xs_wav2=test_x_wav2[i_t*batch_size1:(i_t+1)*batch_size1]
    test_xs_wav2=test_xs_wav2.reshape((batch_size1,nframes,dim))
    test_output2=sess.run(out_all_final,feed_dict={x:test_xs2, y:test_ys2,x_i:test_xs_i2,x_wav:test_xs_wav2,keep_prob:1.0,is_training2:0,batch_size:batch_size1})   
    test_data2[i_t]=test_output2 
    test_label2[i_t]=test_ys2
    if i_t == 0:
        print('test2：')
        print(test_output2)
    if i_t==1:
...
test_save_name2=predict_path+'/mismatch2.mat'
sio.savemat(test_save_name2,{'test_x2':test_data2,'test_y2':test_label2})
